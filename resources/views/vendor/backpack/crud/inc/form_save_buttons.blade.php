@if(isset($saveAction['active']) && !is_null($saveAction['active']['value']))
    <div id="saveActions" class="form-group">
        ...
        @if(!$crud->hasOperationSetting('showCancelButton') || $crud->getOperationSetting('showCancelButton') == true)
            <a href="{{ $crud->hasAccess('list') ? url($crud->route) : url()->previous() }}" class="btn btn-default btn-back"><span class="la la-ban"></span> &nbsp;{{ trans('backpack::crud.cancel') }}</a>
        @endif
...
<script>
    ...
    jQuery(document).ready(function($) {
        ...
        var $updateId = $(form).attr('data-id');
        ...
        function submitForm(form, event, id) {
            ...
            $.ajax({
                ...
                success: function(result) {
                    if (result.success) {
                        let $action = formData.get('_save_action');
                        switch ($action) {
                            case "save_and_edit":
                                window.location = result.referrer_url;
                                break;
                            case "save_and_new":

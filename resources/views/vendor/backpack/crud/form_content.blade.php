...
@else
  <div class="card">
    <div class="card-body row">
      ...
      <img id="imagePreview" src="" alt="Image Preview" style="max-width: 200px; display: none; margin-left: 15px">
    </div>
  </div>
@endif
...
@section('after_scripts')
    ...
    <script>
        $(document).ready(function() {
            if ($('.existing-file').length && !$('.existing-file').hasClass('d-none')) {
                let $src = $('.existing-file a').attr('href');
                previewImageUpload('#imagePreview', $src)
            }

            $('input[name=image]').change(function(e) {
                var reader = new FileReader();
                reader.onload = function(e) {
                    previewImageUpload('#imagePreview', e.target.result)
                };
                reader.readAsDataURL(this.files[0]);
            });

            $('.file_clear_button').click((e) => {
                clearImageUpload('#imagePreview')
            })
        });

        function clearImageUpload(query) {
            $(query).attr('src', '');
            $(query).hide();
        }

        function previewImageUpload(query, src) {
            $(query).attr('src', src);
            $(query).show();
        }
